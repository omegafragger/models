...
# Parameters for implementing dropout in xception network
_APPLY_DROPOUT = True
_DROPOUT_KEEP_RATE = 0.5
...
def xception_module(inputs,
  ...
  Args:
    ...
    dropout: Whether there is a dropout layer at the end of the module
  ...
  with tf.variable_scope(scope, 'xception_module', [inputs]) as sc:
    ...
    if dropout:
      outputs = slim.dropout(outputs,
                             keep_prob=_DROPOUT_KEEP_RATE,
                             is_training=_APPLY_DROPOUT,
                             scope='dropout')
